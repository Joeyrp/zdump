...
const parse_funcs: [7]FnType = .{ parse_num_cols, parse_block_size, parse_dump_test_file, parse_help, parse_page_size, parse_offset, parse_find_target };
...
fn parse_find_target(conf: *Config, at: u32, args: [][:0]u8) !u32 {
    var idx = at;
    if (std.mem.eql(u8, args[idx], "-f")) {
        idx += 1;
        conf.find_target = args[idx];
        return 2;
    }
